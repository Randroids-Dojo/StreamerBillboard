 ...
 * via the dashboard session cookie instead of a ?key= query param, so the
 * AUTH_SECRET is never exposed in a URL.
 */
export async function GET(request: NextRequest) {
  const authSecret = process.env.AUTH_SECRET;
  const cookie = request.cookies.get("sbb_dashboard_auth");

  if (authSecret && cookie?.value !== authSecret) {
